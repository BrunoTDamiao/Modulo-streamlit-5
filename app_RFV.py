...
# Configuração inicial da página da aplicação (deve estar no início do script)
st.set_page_config(
    page_title='RFV',
    layout="wide",
...
@st.cache_data  # Atualizado para st.cache_data
def convert_df(df):
...
def to_excel(df):
    ...
    writer.close()  # Substitua .save() por .close()
...
def main():
    ...
    if data_file_1 is not None:
        ...
        st.write(
            "Um jeito de segmentar os clientes é criando quartis para cada componente do RFV, sendo que o melhor quartil é chamado de 'A', o segundo melhor quartil de 'B', o terceiro melhor de 'C' e o pior de 'D'. O melhor e o pior depende da componente.")
        ...
        dict_acoes = {
            'AAA': 'Enviar cupons de desconto, pedir para indicar nosso produto para algum amigo, ao lançar um novo produto enviar amostras grátis.',
            'DDD': 'Churn! clientes que gastaram pouco e fizeram poucas compras, não fazer nada.',
